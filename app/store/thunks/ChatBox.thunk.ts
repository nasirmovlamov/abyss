...
export const openRooms = createAsyncThunk(types.OPEN_CHATBOX, async (_, { rejectWithValue }) => {
  try {
...
export const reqRoomMessages = createAsyncThunk(
  types.OPEN_ROOM_MESSAGES,
  async (
    { roomId, lastMessageId }: { roomId: number; lastMessageId: number },
    { rejectWithValue },
  ) => {
    try {
...
export const checkRoomChat = createAsyncThunk(
  types.CHECK_ROOM,
  async (opponentID: number, { rejectWithValue }) => {
    try {
