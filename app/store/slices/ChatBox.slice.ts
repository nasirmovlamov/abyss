...
import { ChatBoxState } from '../states/states/ChatBox.state';
import { checkChat, checkRoomChat, loadArchieveMessages, openRooms, sendMessageToRoom } from '../thunks/ChatBox.thunk';
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(checkRoomChat.fulfilled, (state, { payload }) => {
      if (state.rooms[payload.data.id].messages.length === 0) {
        state.rooms[payload.data.id].messages = [
          ...state.rooms[payload.data.id].messages,
    ...
    builder.addCase(sendMessageToRoom.fulfilled, (state, { payload }) => {
      state.rooms[state.openedChatRoomId!].messages = [
        ...state.rooms[state.openedChatRoomId!].messages,
        payload.data,
      ]
    }),
