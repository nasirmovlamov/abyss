...
export default function ChatBox({}: Props): ReactElement {
  ...
  useEffect(() => {
    if (
      ...
      chatRooms[openedChatRoomId].messages.length > 0
    ) {
      dispatch(
  ...
  return (
    <ChatWindow>
      <ChatNav>
        <ChatNavName>{userData!.name}</ChatNavName>{' '}
        <CloseChatBox onClick={closeChatBox}>X</CloseChatBox>
      </ChatNav>

      <ChatMain>
        <ChatRooms>
          {getRooms(chatRooms).map((room: any) => (
            <ChatRoom onClick={() => changeRoomId(room.id)} key={room.id}>
              {room.opponent_user.name}
            </ChatRoom>
          ))}
        </ChatRooms>
        <ChatMessagesTab>
          {openedChatRoomId !== null && (
            <>
              <ChatMessages ref={messagesBoxRef}>
                ...
                {chatRooms[openedChatRoomId].messages.map((message: any) => (
                  <ChatMessage isMe={userData!.id === message.user.id} key={message.id}>
                    {message.content}
                  </ChatMessage>
                ))}
              ...
              <ChatSendMessage>
                ...
                />
                {
                  <button
                    onClick={() =>
                      dispatch(
                        sendMessageToRoom({ roomId: openedChatRoomId, content: userMessage }),
                      )
                    }
                  >
                    Send
