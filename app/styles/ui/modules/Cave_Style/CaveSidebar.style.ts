...
export const CaveSide_section_Sty = styled.button<{
  ...
  background-color: ${({ selected, theme }) =>
    selected ? theme.backgrounds.background3 : 'transparent'};
  ...
  border-left: 2px solid
    ${({ selected, theme }) => (selected ? theme.sidecolors.sidecolor4 : 'transparent')};
  display: flex;
  align-items: center;
  column-gap: 10px;
  font-size: 18px;
  ...
  &:hover {
    background-color: ${({ selected, theme }) =>
      selected ? theme.backgrounds.background3 : theme.backgrounds.background2};
  }
`
