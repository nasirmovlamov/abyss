    ...
    builder.addCase(getAnswers.fulfilled, (state, { payload }) => {
      ...
      if (payload === null) {
        ...
        if (payload.next) {
          ...
          if (
            topAnswers.answers.length + downAnswers.answers.length ===
              state.answersData.totalPage &&
            state.answersData.topPage > 1
          ) {
            topAnswers.status = 'idle'
            downAnswers.status = 'idle'
          ...
          if (
            downAnswers.answers.length + topAnswers.answers.length ===
            state.answersData.totalPage
          ) {
            downAnswers.status = 'idle'
            topAnswers.status = 'idle'
      ...
      builder.addCase(getAnswers.pending, (state, { payload }) => {
        ...
        const downAnswers = state.answersData.downAnswers
        if (
          topAnswers.answers.length + downAnswers.answers.length === state.answersData.totalPage &&
          state.answersData.topPage > 1
        ) {
          topAnswers.status = 'idle'
          downAnswers.status = 'idle'
...
export const submitted_answer = (state: RootState) =>
  state.questionReducer.answersData.submittedAnswer
export const down_answers = (state: RootState) =>
  state.questionReducer.answersData.downAnswers.answers
export const top_answers = (state: RootState) =>
  state.questionReducer.answersData.topAnswers.answers
export const top_answers_status = (state: RootState) =>
  state.questionReducer.answersData.topAnswers.status
export const down_answers_status = (state: RootState) =>
  state.questionReducer.answersData.downAnswers.status
export const mentioned_users_at_anwser_submit = (state: RootState) =>
  state.questionReducer.answerSubmitData.mentionedUsers
export const linked_products_at_anwser_submit = (state: RootState) =>
  state.questionReducer.answerSubmitData.linkedProducts
export const submit_answer_content = (state: RootState) =>
  state.questionReducer.answerSubmitData.content
...
export const current_page_linked_products = (state: RootState) =>
  state.questionReducer.linkedProductsData.current_page
export const total_linked_products = (state: RootState) =>
  state.questionReducer.linkedProductsData.total
export const last_page_linked_products = (state: RootState) =>
  state.questionReducer.linkedProductsData.last_page
export const linked_products_for_answers_of_question = (state: RootState) =>
  state.questionReducer.linkedProductsData.linkedProducts
export const linked_products_status = (state: RootState) =>
  state.questionReducer.linkedProductsData.status

export const mentions_of_linked_product = (state: RootState) =>
  state.questionReducer.mentionsOfLinkedProduct
