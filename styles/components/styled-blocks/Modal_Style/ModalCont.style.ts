...
export const CreateThread_Tab = styled.div<{stage:boolean , validated:boolean}>`
  width: 30px;
  height: 30px;
  border-radius: 50%;
  border: 5px solid ${({theme , stage}) => stage? theme.sidecolors.sidecolor4 : theme.sidecolors.sidecolor1};
`

export const CreateThread_Tab_Seperator = styled.div<{validated:boolean}>`
  width: 100px;
  height: 1px;
  background-color: ${({theme , validated}) => validated? theme.sidecolors.sidecolor4 : theme.sidecolors.sidecolor1};
`
