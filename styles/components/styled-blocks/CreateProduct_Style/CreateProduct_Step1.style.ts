...
export const SelectThreadType = styled.button<{ selected: boolean }>`
  ...
  .textBlock {
    ...
    outline: 2px solid
      ${({ theme, selected }) => (selected ? theme.sidecolors.sidecolor4 : ' transparent')};
    padding: 15px;
    ...
    background-color: ${({ theme, selected }) =>
      selected ? theme.backgrounds.background2 : theme.backgrounds.background1};
    p {
      font-size: 15px;
      color: ${({ theme }) => theme.texts.text1};
      width: 100%;
  ...
  &:hover {
    ...
    .textBlock {
      background-color: ${({ theme, selected }) =>
        selected ? theme.backgrounds.background2 : theme.backgrounds.background2};
