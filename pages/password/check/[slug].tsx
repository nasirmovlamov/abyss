...
import { change_password_data, form_change_password_data, user_data, user_forget_pass_check_token } from '../../../app/feature/User.slice'
...
const ForgotPassChange = (props: Props) => {
    ...
    const userForgetPassCheckToken: { status: string, message: string, errors: any } = useAppSelector(user_forget_pass_check_token)
    const { changePass, change_pass_onChange, change_pass_onBlur } = useChangePass({ status: userForgetPassCheckToken.status, errors: userForgetPassCheckToken.errors })
    ...
    if (userForgetPassCheckToken.status === "pending") {
        ...
        return (
            <div style={{ paddingTop: '25px', paddingBottom: '25px', rowGap: "10px", width: "100%", height: "auto", backgroundColor: "black", color: "white", display: "flex", alignItems: "center", marginTop: "50px", marginBottom: "50px", flexDirection: "column" }}>
                ...
                }
                {
                    ...
                    <ChangePassSTY.ForgetPassForm action="" onSubmit={changePass}>
                        ...
                        <div className="form-group">
                            ...
                            <input type="password" name='password' value={formChangePasswordData.data.password} onChange={change_pass_onChange} onBlur={change_pass_onBlur} />
                            <span style={{ color: "red" }}>{formChangePasswordData.errors.password.errorMessage}</span>
                        </div>

                        <div className="form-group">
                            <label htmlFor="password_confirmation" >Confirm Password</label>
                            <input type="password" name='password_confirmation' value={formChangePasswordData.data.password_confirmation} onChange={change_pass_onChange} onBlur={change_pass_onBlur} />
                            <span style={{ color: "red" }}>{formChangePasswordData.errors.password_confirmation.errorMessage}</span>
                        </div>

                        <div className="backend-errors">
                            {changePasswordData.errors &&
                                Object.keys(changePasswordData.errors).map((key: any) =>
                                    <p key={key} style={{ color: "red" }}>{changePasswordData.errors[key]}</p>
                                )
                            }
                        </div>

                        <button type='submit' disabled={!formChangePasswordData.isValid}>submit</button>
