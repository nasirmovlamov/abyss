...
import { ProductCreateStep3OnBlurs, ProductCreateStep3OnChanges, product_create_step3_data } from '../../../../../app/feature/CreateProductFeatures/CreateProductSlice'
    ...
    return (
        <div>
            ...
            <div style={{display:"flex" , flexDirection:"column" , rowGap:"25px"}}>
                ...
                {iterations_of_product.map((iteration,index)=>
                    <div style={{display:'flex' , flexDirection:'column' , rowGap:"5px" , border:'1px solid black' , padding:"5px"}} key={iteration.id}>
                        <button onClick={() => productIterationDelete(iteration.id)}>del</button>
                        <label htmlFor="">name of iteration</label>
                        <input onBlur={() => productIterationOnBlurHandleName(iteration.id)}  type="text" value={iteration.iteration_name} onChange={(e) => productIterationOnChangeHandleName(iteration.id , e.target.value)}/>
                        <label htmlFor="">code of iteration</label>
                        <input onBlur={() => productIterationOnBlurHandleCode(iteration.id)} type="text" value={iteration.iteration_code} onChange={(e) => productIterationOnChangeHandleCode(iteration.id , e.target.value)}/>
                        <label htmlFor="">note of iteration</label>
                        <input onBlur={() => productIterationOnBlurHandleNote(iteration.id)} type="text" value={iteration.iteration_note} onChange={(e) => productIterationOnChangeHandleNote(iteration.id , e.target.value)}/>
                        {(iteration.validators.areAllFilled.valid === 'failed' && 
                            <>
                            {!iteration.validators.isCodeFilled.valid &&  <div>{iteration.validators.isCodeFilled.message}</div>}
                            {!iteration.validators.isNameFilled.valid && <div>{iteration.validators.isNameFilled.message}</div>}
                            {!iteration.validators.isNoteFilled.valid &&<div>{iteration.validators.isNoteFilled.message}</div>}
                            </>
                        )}
                    </div>
                )}
